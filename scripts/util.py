...
def movie_time(name,
    ...
    Ntime = len(time)
    vmin0 = np.log10(max([np.min(star[:, ww]), np.min(plan[:, ww]), np.min(disk[:, ww])]))
    ...
    for i in range(Ntime):
        ...
        sys.stdout.flush()
        ext = ((imgs.shape[2]-1)//2*pixscale+0.5*pixscale)*wave[ww]/diam*rad2mas
        f, ax = plt.subplots(1, 4, figsize=(4.8*4, 3.6*1))
        p0 = ax[0].imshow(np.fliplr(np.log10(imgs[i, ww])), origin='lower', extent=(-ext, ext, -ext, ext), vmin=vmin0, vmax=vmax0)
        ...
        p1 = ax[1].imshow(np.fliplr(np.log10(star[i, ww])), origin='lower', extent=(-ext, ext, -ext, ext), vmin=vmin1, vmax=vmax1)
        ...
        ax[1].set_title('Star')
        p2 = ax[2].imshow(np.fliplr(np.log10(plan[i, ww])), origin='lower', extent=(-ext, ext, -ext, ext), vmin=vmin2, vmax=vmax2)
        ...
        p3 = ax[3].imshow(np.fliplr(np.log10(disk[i, ww])), origin='lower', extent=(-ext, ext, -ext, ext), vmin=vmin3, vmax=vmax3)
...
def movie_wave(name,
    ...
    for i in range(Nwave):
        ...
        sys.stdout.flush()
        ext = ((imgs.shape[2]-1)//2*pixscale+0.5*pixscale)*wave[i]/diam*rad2mas
        f, ax = plt.subplots(1, 4, figsize=(4.8*4, 3.6*1))
        p0 = ax[0].imshow(np.fliplr(np.log10(imgs[ww, i])), origin='lower', extent=(-ext, ext, -ext, ext), vmin=vmin0, vmax=vmax0)
        c0 = plt.colorbar(p0, ax=ax[0])
        c0.set_label('$\log_{10}$(ph/s/pix)', rotation=270, labelpad=20)
        ax[0].invert_xaxis()
        ax[0].set_xlabel('$\Delta$RA [mas]')
        ax[0].set_ylabel('$\Delta$DEC [mas]')
        ax[0].set_title('Scene')
        p1 = ax[1].imshow(np.fliplr(np.log10(star[ww, i])), origin='lower', extent=(-ext, ext, -ext, ext), vmin=vmin1, vmax=vmax1)
        c1 = plt.colorbar(p1, ax=ax[1])
        c1.set_label('$\log_{10}$(ph/s/pix)', rotation=270, labelpad=20)
        ax[1].invert_xaxis()
        ax[1].set_xlabel('$\Delta$RA [mas]')
        ax[1].set_ylabel('$\Delta$DEC [mas]')
        ax[1].set_title('Star')
        p2 = ax[2].imshow(np.fliplr(np.log10(plan[ww, i])), origin='lower', extent=(-ext, ext, -ext, ext), vmin=vmin2, vmax=vmax2)
        c2 = plt.colorbar(p2, ax=ax[2])
        c2.set_label('$\log_{10}$(ph/s/pix)', rotation=270, labelpad=20)
        ax[2].invert_xaxis()
        ax[2].set_xlabel('$\Delta$RA [mas]')
        ax[2].set_ylabel('$\Delta$DEC [mas]')
        ax[2].set_title('Planets')
        p3 = ax[3].imshow(np.fliplr(np.log10(disk[ww, i])), origin='lower', extent=(-ext, ext, -ext, ext), vmin=vmin3, vmax=vmax3)
        c3 = plt.colorbar(p3, ax=ax[3])
        c3.set_label('$\log_{10}$(ph/s/pix)', rotation=270, labelpad=20)
        ax[3].invert_xaxis()
        ax[3].set_xlabel('$\Delta$RA [mas]')
...
def movie_make(name,
    ...
    out = cv2.VideoWriter(path+'_movie.mp4', cv2.VideoWriter_fourcc(*'MP4V'), 24, size)
...
def proj_sa(xx,
    ...
    # Convert.
    rho = np.sqrt(xx_inc**2+yy_inc**2)
    phi = np.rad2deg(np.arccos(yy_rot*np.tan(np.deg2rad(inc))/np.sqrt(xx_inc**2+yy_inc**2)))
    sgn = np.sign(phi) # symmetry along Omega
